...
qrmon <-
  ...
  QRMonQuantileRegression( df = 12, degree = 2, quantiles = seq(0.2,0.8,0.2) ) %>% 
  QRMonPlot( dataPointsColor = "gray10", regressionCurvesColor = ~ RegressionCurve )
```
...
qrmon <-
  ...
  QRMonQuantileRegression( df = 9, quantiles = c(0.05,0.95) ) %>% 
  QRMonOutliers() %>% 
...
qrmon <-
  ...
  QRMonSetRegressionObjects(NULL) %>% 
  QRMonQuantileRegression( df = 9, quantiles = c(0.01, 1:7/8, 0.98) ) %>% 
  QRMonPlot()
```

```{r}
qrmon <- qrmon %>% QRMonSimulate(200)
