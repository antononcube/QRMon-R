...
test_that( "Rescaling produces points in [0,1].", {
  expect_true( min(QRMonTakeData(qrObj1)$Time) == 0 && max(QRMonTakeData(qrObj1)$Time) == 1 )

  expect_true( min(QRMonTakeData(qrObj2)$Time) == 0 && max(QRMonTakeData(qrObj2)$Time) == 1 )
...
test_that( "Rescaling of a constant column produces a column of 0's.", {
  expect_true( min(QRMonTakeData(qrObj3)$Time) == 0 && max(QRMonTakeData(qrObj3)$Time) == 0 )
})
